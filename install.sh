...
curl -sL  https://github.com/percona/percona-everest-cli/releases/download/${latest_release}/everest-cli-$os-$arch -o everest-cli
chmod +x everest-cli
...
echo "You can run ./everest-cli for the wizard setup"
echo ""
echo "Also you can use --kubeconfig to specify a path for kubeconfig you want to use"
./everest-cli install operators --backup.enable=false --everest.endpoint=http://127.0.0.1:8080  --monitoring.enable=false --operator.mongodb=true --operator.postgresql=true --operator.xtradb-cluster=true --skip-wizard
