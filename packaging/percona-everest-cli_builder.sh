...
install_go() {
    ...
    if [ x"$ARCH" = "xx86_64" ]; then
        ...
        wget https://go.dev/dl/go1.20.7.linux-arm64.tar.gz
        tar -C /usr/local -xzf go1.20.7.linux-arm64.tar.gz
    fi
...
get_sources(){
    ...
    echo "PRODUCT=${PRODUCT}" > everest-cli.properties

    PRODUCT_FULL=${PRODUCT}-${VERSION}
    echo "PRODUCT_FULL=${PRODUCT_FULL}" >> everest-cli.properties
    echo "VERSION=${VERSION}" >> everest-cli.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> everest-cli.properties
    echo "BUILD_ID=${BUILD_ID}" >> everest-cli.properties
    ...
    echo 9 > compat
    echo "percona-everest-cli (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    echo "" >> changelog
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH}/${REVISION}/${BUILD_ID}" >> everest-cli.properties
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name 'percona-everest-cli*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name 'percona-everest-cli*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
...
get_deb_sources(){
    param=$1
    echo $param
    FILE=$(basename $(find $WORKDIR/source_deb -name "percona-everest-cli*.$param" | sort | tail -n1))
    if [ -z $FILE ]
    then
        FILE=$(basename $(find $CURDIR/source_deb -name "percona-everest-cli*.$param" | sort | tail -n1))
        if [ -z $FILE ]
        then
...
build_srpm(){
    ...
    cp -av packaging/rpm/percona-everest-cli.spec rpmbuild/SPECS
    #
    mv -fv ${TARFILE} ${WORKDIR}/rpmbuild/SOURCES
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" rpmbuild/SPECS/percona-everest-cli.spec
...
build_rpm(){
    ...
    SRC_RPM=$(basename $(find $WORKDIR/srpm -name 'percona-everest-cli*.src.rpm' | sort | tail -n1))
    if [ -z $SRC_RPM ]
    then
        SRC_RPM=$(basename $(find $CURDIR/srpm -name 'percona-everest-cli*.src.rpm' | sort | tail -n1))
        if [ -z $SRC_RPM ]
        then
...
build_source_deb(){
    ...
    #
    TARFILE=$(basename $(find . -name 'percona-everest-cli*.tar.gz' | sort | tail -n1))
    ...
    cd ${BUILDDIR}
    rm -rf .git
    dch -D unstable --force-distribution -v "${VERSION}-${RELEASE}" "Update to new everest-cli version ${VERSION}"
    dpkg-buildpackage -S
    cd ../
...
build_deb(){
    ...
    export ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
    #
    echo "DEBIAN=${DEBIAN}" >> everest-cli.properties
    echo "ARCH=${ARCH}" >> everest-cli.properties
    #
...
REVISION=0
BRANCH="main"
REPO="http://github.com/percona/percona-everest-cli"
PRODUCT=percona-everest-cli
DEBUG=0
